...
import { Check, Plus, Trash, Upload, X } from "../../components/Icons.jsx";
import "./AdminTab.css";
  ...
  return (
    <div className="bug-att">
      {att.kind === "video" ? (
        ...
        <a href={url} target="_blank" rel="noopener noreferrer" title={att.original_name || ""}>
          <img className="bug-att__media" src={url} alt={att.original_name || "צילום מסך"} loading="lazy" />
        </a>
      )}
                ...
                <div className="bug-card__head">
                  ...
                  <div className="bug-card__actions">
                    <button type="button" className="btn btn--ghost btn--sm" onClick={() => toggleStatus(bug)}>
                      {bug.status === "open" ? <><Check /> סגור באג</> : "פתח מחדש"}
                    </button>
